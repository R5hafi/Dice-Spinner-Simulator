//Name: Ryan Shafi
//Student Number: 501 167 088
import java.util.ArrayList;

/*
 * An AudioBook is a type of AudioContent.
 * It is a recording made available on the internet of a book being read aloud by a narrator
 *
 */
public class AudioBook extends AudioContent
{
	public static final String TYPENAME =	"AUDIOBOOK";

	private String author;
	private String narrator;
	private ArrayList<String> chapterTitles;
	private ArrayList<String> chapters;
	private int currentChapter = 0;

	// constructor for AudioBook class
	// title, year, id, type, audioFile, length inherited from super class AudioContent
	// author, narrator, chapterTitles, and chapters are instance variables unique to audioBook class
	public AudioBook(String title, int year, String id, String type, String audioFile, int length,
					 String author, String narrator, ArrayList<String> chapterTitles, ArrayList<String> chapters)
	{
		// Make use of the constructor in the super class AudioContent.
		// Initialize additional AudioBook instance variables.
		super(title, year, id, type, audioFile, length);
		this.author = author;
		this.narrator = narrator;
		this.chapterTitles = chapterTitles;
		this.chapters = chapters;
	}

	public String getType()
	{
		return TYPENAME;
	}

	// Print information about the audiobook. First print the basic information of the AudioContent
	// by making use of the printInfo() method in superclass AudioContent and then print author and narrator
	// see the video
	public void printInfo()
	{
		// super class method printInfo is used for inherited variables
		// then instance variables author and narrator are printed
		super.printInfo();
		System.out.println("Author: "+this.author+" Narrated by: "+this.narrator);
	}

	// Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list)
	// followed by the current chapter (from chapters array list)
	// Then make use of the the play() method of the superclass
	public void play()
	{
		// audioFile is set using super class method setAudioFile
		// it is set to the chapter title followed by current chapter on the next line
		// then super class play method is called to display chapter content
		super.setAudioFile(this.chapterTitles.get(currentChapter)+"\n"+this.chapters.get(currentChapter));
		super.play();
	}

	// Print the table of contents of the book - i.e. the list of chapter titles
	// See the video
	public void printTOC()
	{
		// TOC is the list of chapter titles
		// the chapter titles are looped through for each chapter
		// their number is displayed by adding 1 to the loop index
		// chapter number is followed by a dot then title,
		// and then a new line is added for format
		for(int i = 0; i < this.chapterTitles.size(); i++){
			int index = i + 1;
			System.out.println("Chapter "+index+". "+this.chapterTitles.get(i)+"\n");
		}
	}

	// Select a specific chapter to play - nothing to do here
	public void selectChapter(int chapter)
	{
		if (chapter >= 1 && chapter <= chapters.size())
		{
			currentChapter = chapter - 1;
		}
	}

	//Two AudioBooks are equal if their AudioContent information is equal and both the author and narrators are equal
	public boolean equals(Object other)
	{
		// similar to song class, other object must be casted to AudioBook for comparison
		// then the function returns the result of comparing inherited variables, author, and narrator
		AudioBook a = (AudioBook)other;
		return super.equals(other) && this.author.equals(a.author)  && this.narrator.equals(a.narrator);
	}

	public int getNumberOfChapters()
	{
		return chapters.size();
	}

	public String getAuthor()
	{
		return author;
	}

	public void setAuthor(String author)
	{
		this.author = author;
	}

	public String getNarrator()
	{
		return narrator;
	}

	public void setNarrator(String narrator)
	{
		this.narrator = narrator;
	}

	public ArrayList<String> getChapterTitles()
	{
		return chapterTitles;
	}

	public void setChapterTitles(ArrayList<String> chapterTitles)
	{
		this.chapterTitles = chapterTitles;
	}

	public ArrayList<String> getChapters()
	{
		return chapters;
	}

	public void setChapters(ArrayList<String> chapters)
	{
		this.chapters = chapters;
	}

}
